...
class HalfordsSpider(scrapy.Spider):
    name = "halfords"

    def start_requests(self):
        yield scrapy.Request(
            url = 'https://www.halfords.com/sitemap_index.html'
        )
    ...
    def parse_category(self, response: Response) -> Generator:
        for product in response.css('article.product-item'):
            ...
            print("URL for scrapy request for product is ", product.css('a::attr(href)').get())
            yield scrapy.Request(
    ...
    def parse_product(self, response: Response, data_package: Dict[str, str]) -> Generator:
        product_data = {
            ...
            'product_brand': 'Halfords',
            'product_name': response.css('a.title::text').get(),
            'product_retail_price': response.css('span.b-price__sale::text').get(),
            'product_current_price': response.css('span.b-price__sale::text').get(),
            'product_description': response.xpath("//meta[@name='description']/@content").extract_first(),
